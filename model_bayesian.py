...
class BayesianConv2d(nn.Module):
    ...
    def forward(self, input, sample=True):
        if self.training or sample:
            W_eps = torch.empty(self.W_mu.size()).normal_(0, 1).to(cuda_name)
            self.W_sigma2 = torch.log1p(torch.exp(self.W_logsigma2))
            weight = self.W_mu + W_eps * self.W_sigma2.sqrt()

            if self.use_bias:
                bias_eps = torch.empty(self.bias_mu.size()).normal_(0, 1).to(cuda_name)
...
class BBBConv2d(ModuleWrapper):
    def __init__(self, in_channels, out_channels, kernel_size,
                 stride=1, padding=1, dilation=1, bias=True, priors=None, device=cuda_name):
    ...
    def forward(self, input, sample=True):
        if self.training or sample:
            W_eps = torch.empty(self.W_mu.size()).normal_(0, 2).to(self.device)
            self.W_sigma = torch.log1p(torch.exp(self.W_rho))
            weight = self.W_mu + W_eps * self.W_sigma

            if self.use_bias:
                bias_eps = torch.empty(self.bias_mu.size()).normal_(0, 2).to(self.device)
...
class DoubleBayesianConv(nn.Module):
    def __init__(self, in_channels, out_channels, mid_channels=None):
        super().__init__()
        if not mid_channels:
            mid_channels = out_channels
        self.double_conv1 = nn.Sequential(
            BBBConv2d(in_channels, mid_channels, kernel_size=3, bias=False, padding=0),
            nn.BatchNorm2d(mid_channels),
            nn.ReLU(inplace=True),
        )
        self.double_conv2 = nn.Sequential(
            BBBConv2d(mid_channels, out_channels, kernel_size=3, bias=False, padding=0),
...
class DoubleConv(nn.Module):
    ...
    def forward(self, x):
        ...
        # Yemre ekledi: mirrored top and bottom rows
        # top_row = x[:,:,0,:].unsqueeze(2)
        # top_row_left_half = top_row[:,:,0,:x.shape[3]//2].unsqueeze(2)
        # top_row_right_half = top_row[:,:,0,x.shape[3]//2:].unsqueeze(2)
        # top_row_mirror = torch.cat([top_row_right_half, top_row_left_half], dim=3)

        # bottom_row = x[:,:,x.shape[2] - 1,:].unsqueeze(2)
        # bottom_row_left_half = bottom_row[:,:,0,:x.shape[3]//2].unsqueeze(2)
        # bottom_row_right_half = bottom_row[:,:,0,x.shape[3]//2:].unsqueeze(2)
        # bottom_row_mirror = torch.cat([bottom_row_right_half, bottom_row_left_half], dim=3)

        # zero_pad[:,:,0,:] = top_row_mirror.squeeze(2)
        # zero_pad[:,:,x.shape[2]-1,:] = bottom_row_mirror.squeeze(2)
...
class BayesianUNetPP(nn.Module):
    def __init__(self, num_classes, input_channels=3, device=cuda_name):
        super().__init__()
        ...
        #self.final = BayesianLinear(1*192*288, 192*288)
...
class BayesianUNetPP_2Layers(nn.Module):
    def __init__(self, num_classes, input_channels=3, device=cuda_name):
        super().__init__()
        ...
        self.conv0_3 = DoubleBayesianConv(nb_filter[0]*3+nb_filter[1], nb_filter[0], nb_filter[0])
...
class BayesianUNetPP_3Layers(nn.Module):
    def __init__(self, num_classes, input_channels=3, device=cuda_name):
        super().__init__()
        ...
        self.conv0_2 = DoubleBayesianConv(nb_filter[0]*2+nb_filter[1], nb_filter[0], nb_filter[0])
        self.conv1_2 = DoubleBayesianConv(nb_filter[1]*2+nb_filter[2], nb_filter[1], nb_filter[1])
