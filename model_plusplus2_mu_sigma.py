...
class NestedUNet2(nn.Module):
    def __init__(self, num_classes, input_channels=3, deep_supervision=False, **kwargs):
        ...
        self.batchnorm = torch.nn.BatchNorm2d(nb_filter[0])
        ...
        if self.deep_supervision:
            ...
            self.final = nn.Conv2d(nb_filter[0], 2, kernel_size=3)
    ...
    def forward(self, input):
        ...
        x0_3 = self.batchnorm(x0_3)
        zero_pad = F.pad(input=x0_3, pad=(0, 0, 1, 1), mode="constant", value=0)
        circular_pad = F.pad(input=zero_pad, pad=(1, 1, 0, 0), mode="circular")
        output = self.final(circular_pad)
        mu = output[:, 0]
        sigma = self.softplus(output[:, 1]) + 1e-6
        return mu, sigma
