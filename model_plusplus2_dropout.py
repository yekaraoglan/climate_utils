...
class NestedUNet2(nn.Module):
    ...
    def forward(self, input):
        p = 0.1
        x0_0 = F.dropout(self.conv0_0(input)) #down
        x1_0 = F.dropout(self.conv1_0(self.pool(x0_0))) #down
        x0_1 = F.dropout(self.conv0_1(torch.cat([x0_0, self.up(x1_0)], 1)), p=p) # inter

        x2_0 = F.dropout(self.conv2_0(self.pool(x1_0))) #down
        x1_1 = F.dropout(self.conv1_1(torch.cat([x1_0, self.up(x2_0)], 1)))#inter
        x0_2 = F.dropout(self.conv0_2(torch.cat([x0_0, x0_1, self.up(x1_1)], 1)), p=p) #inter

        x3_0 = F.dropout(self.conv3_0(self.pool(x2_0))) #down 
        x2_1 = F.dropout(self.conv2_1(torch.cat([x2_0, self.up(x3_0)], 1)))#up
        x1_2 = F.dropout(self.conv1_2(torch.cat([x1_0, x1_1, self.up(x2_1)], 1)))#up
        x0_3 = F.dropout(self.conv0_3(torch.cat([x0_0, x0_1, x0_2, self.up(x1_2)], 1)), p=p) #up
